The Sidequest That Wasn’t a Sidequest
I told myself it was a detour.
A week ago I was supposed to be adding features to ERES—my chemical incident simulation platform. Instead, I found myself staring at a stack of infrastructure problems I didn’t fully understand. Services failing in ways I couldn’t explain. Memory ceilings I kept bumping into. Permission errors that seemed to appear at random. I could patch each one individually and move on, but something told me that was the wrong approach.
In emergency management, we have a name for what I was doing: treating symptoms instead of the incident. So I stopped. And I built a lab.
...
ERES is designed to be a tool emergency managers can trust in high-stakes training environments. That means it can’t just work when conditions are perfect. It has to work when someone is running a full-scale exercise, multiple users are hitting the system at once, and the network is doing unexpected things.
AI doesn’t fail in isolation. It fails inside infrastructure. Before this week, I understood that intellectually. Now I understand it operationally—I’ve watched it happen, diagnosed it, and fixed it. That’s a different kind of knowledge.
A tool built by someone who’s only ever seen the AI layer is a tool that will surprise you in production. I don’t want to build that tool.
On Sidequests
